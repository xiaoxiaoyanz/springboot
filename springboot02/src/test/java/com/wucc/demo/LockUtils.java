...
public class LockUtils {
	 ...
	 *
	 * @param lock_key 							锁key
	 * @param locktimeout(毫秒) 持有锁的有效时间，防⽌死锁
	 * @param gettimeout(毫秒) 获取锁的超时时间，这个时间内获取不到将重试
	 * @return
	 */
	public static boolean lock(String lock_key, long locktimeout, int
			gettimeout) throws Exception {
		log.info("start");
		...
		while (true) {
			...
			if (Objects.isNull(lockModel)) {
				...
				if ("".equals(reqid)) {
					...
					//如果request_id和表中request_id⼀样表⽰锁被当前线程持有者，此时需要加重⼊锁
	...
	public static void unlock(String lock_key) throws Exception {
		...
		LockModel lockModel = LockUtils.get(lock_key);
		//当前线程requestId和库中request_id⼀致 && lock_count>0，表⽰可以释放锁
